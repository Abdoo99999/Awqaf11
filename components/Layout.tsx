...
// مكون عنصر القائمة - رابط مباشر بدون شروط
...
const Layout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  // إذا لم يسجل الدخول، اعرض المحتوى مباشرة (لصفحة الدخول)
  if (!isAuthenticated) {
      return <>{children}</>;
  }

  // تعريف الروابط والصلاحيات
  const allNavItems = [
  ...
  return (
    <div className="flex h-screen bg-gray-50 dark:bg-gray-900 overflow-hidden font-sans text-right" dir="rtl">
      ...
      <aside className={`fixed inset-y-0 right-0 z-50 w-64 bg-navy-900 dark:bg-black text-white transform transition-transform duration-300 ease-in-out lg:translate-x-0 lg:static lg:inset-0 shadow-2xl ${sidebarOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        ...
        <nav className="p-4 space-y-2 flex-1 overflow-y-auto mt-2">
          {navItems.map((item) => (
            <SidebarItem 
                ...
                // تمت إزالة دالة onClick الخاصة بالحماية، الآن الانتقال حر ومباشر
            />
          ))}
          <button 
            onClick={logout}
        ...
        {/* تذييل القائمة - تمت إضافة اسم الدكتور */}
        <div className="absolute bottom-0 w-full p-4 border-t border-navy-800 text-xs text-gray-400 text-center bg-navy-900/50">
          <p>{settings.orgName}</p>
          <p className="mt-2 text-gold-500 font-bold">إشراف: د. عبدالرحمن النوفلي</p>
        </div>
      </aside>
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <main className="flex-1 overflow-y-auto p-4 lg:p-8 relative dark:text-gray-100">
          ...
          <div className="hidden print:block fixed bottom-0 left-0 right-0 text-center p-4 border-t text-sm">
             ...
             <p className="font-bold">إشراف: د. عبدالرحمن النوفلي</p>
          </div>
        </main>
      </div>

      {/* غطاء للقائمة في الموبايل */}
      {sidebarOpen && (
